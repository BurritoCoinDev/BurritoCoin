...
#ifndef BURRITOCOIN_PROTOCOL_H
#define BURRITOCOIN_PROTOCOL_H
...
/**
 * BurritoCoin protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // set by all BurritoCoin Core non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // BurritoCoin Core does not support this but a patch set called BurritoCoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // BurritoCoin Core nodes used to support this by default, without advertising this bit,
    ...
    // burritocoin-development mailing list. Remember that service bits are just
...
enum GetDataMsg : uint32_t {
    ...
    MSG_MWEB_HEADER = 8 | MSG_MWEB_FLAG,            //!< MWEB protocol message
    MSG_MWEB_LEAFSET = 9 | MSG_MWEB_FLAG,           //!< MWEB protocol message
};
