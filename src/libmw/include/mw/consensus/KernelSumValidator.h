...
private:
        ...
        const int64_t coins_added)
    {
        ...
        if (coins_added > 0) {
            ...
            // Avoid std::abs(INT64_MIN) which is undefined behavior.
            // Negate via unsigned arithmetic and cast back, which is well-defined
            // for all int64_t inputs including INT64_MIN. coins_added is always
            // bounded by MoneyRange() upstream, but this is defense-in-depth.
            const uint64_t magnitude = -static_cast<uint64_t>(coins_added);
            sum_utxo_commitment = Pedersen::AddCommitments(
                { sum_utxo_commitment, Commitment::Transparent(static_cast<int64_t>(magnitude)) }
            );
        }
