...
#include <qt/burritocoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BurritoCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BurritoCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new BurritoCoinAddressCheckValidator(parent));
}

bool parseBurritoCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no burritocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("burritocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BurritoCoinUnits::parse(BurritoCoinUnits::BRTO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBurritoCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBurritoCoinURI(uriInstance, out);
}

QString formatBurritoCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("burritocoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BurritoCoinUnits::format(BurritoCoinUnits::BRTO, info.amount, false, BurritoCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBurritoCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BURRITOCOIN_CONF_FILENAME));
    ...
    /* Open burritocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BurritoCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BurritoCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BurritoCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BurritoCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "burritocoin.desktop";
    return GetAutostartDir() / strprintf("burritocoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a burritocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BurritoCoin\n";
        else
            optionFile << strprintf("Name=BurritoCoin (%s)\n", chain);
