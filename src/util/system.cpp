...
const char * const BURRITOCOIN_CONF_FILENAME = "burritocoin.conf";
const char * const BURRITOCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/burritocoin/burritocoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //burritocoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BURRITOCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "burritocoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BurritoCoin
    // macOS: ~/Library/Application Support/BurritoCoin
    // Unix-like: ~/.burritocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BurritoCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BurritoCoin";
#else
    // Unix-like
    return pathRet / ".burritocoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BURRITOCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BurritoCoin Core copyright is not removed by accident
    if (copyright_devs.find("BurritoCoin Core") == std::string::npos) {
        if (strPrefix.find("2011") != std::string::npos) {
            std::string strYear = strPrefix;
            strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
            strCopyrightHolders += "\n" + strYear + "The BurritoCoin Core developers";
        }
    }
