...
class WalletTest(BurritoCoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(txout['value'], 10)
        txout = self.nodes[0].gettxout(txid=confirmed_txid, n=confirmed_index, include_mempool=True)
        assert_equal(txout['value'], 10)

        # Send 21 BRTO from 0 to 2 using sendtoaddress call.
        ...
        # Send 10 BRTO with subtract fee from amount
        ...
        # Sendmany 10 BRTO with subtract fee from amount
        ...
        usp = self.nodes[1].listunspent(query_options={'minimumAmount': '9.998'})[0]
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid BurritoCoin address
            assert_raises_rpc_error(-5, "Invalid BurritoCoin address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid BurritoCoin address
            assert_raises_rpc_error(-5, "Invalid BurritoCoin address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid BurritoCoin address or script", self.nodes[0].importaddress, "invalid")
