...
The BurritoCoin Core daemon can be configured to act as a trusted "border
router", implementing the burritocoin wire protocol and relay, making
...
The ZeroMQ feature in BurritoCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building burritocoind:
...
For instance:
    ...
    $ burritocoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/burritocoind.tx.raw \
...
These options can also be provided in burritocoin.conf.
...
From the perspective of burritocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into burritocoind directly. Furthermore, no information is
...
using. BurritoCoind appends an up-counting sequence number to each
