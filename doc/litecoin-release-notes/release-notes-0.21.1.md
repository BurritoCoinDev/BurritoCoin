...
Summary:
==============
In this update to BurritoCoin Core, Taproot has been activated, enhancing BurritoCoin’s security, privacy and scalability. BurritoCoin Core 0.21.1 is the most significant update in years - with performance improvements, new features (like Taproot), bug fixes and updated translations - and sets the foundations for MWEB’s imminent arrival. 

BurritoCoin Core 0.21.1 is available now, right here: <https://download.burritocoin.org/burritocoin-0.21.1/>.

As always, we welcome the expertise of the community. To report bugs please use the issue tracker at GitHub - <https://github.com/burritocoin-project/burritocoin/issues > - and to get security and update notifications subscribe via <https://groups.google.com/forum/#!forum/burritocoin-dev>. 

Anyone can contribute to BurritoCoin Core. Please scroll down to ‘How to contribute to BurritoCoin Core’ for details.

By downloading or upgrading BurritoCoin Core, you’re helping to secure the network, so thank you and enjoy the upgrade!


How to upgrade: 
==============

Firstly, thank you for running BurritoCoin Core and helping secure the network!

As you’re running an older version of BurritoCoin Core, shut it down. Wait until it’s completely shut down  - which might take a few minutes for older versions - then follow these simple steps:
For Windows: simply run the installer 
For Mac: copy over to `/Applications/BurritoCoin-Qt` 
For Linux: copy cover `burritocoind`/`burritocoin-qt`.

NB: upgrading directly from an ‘end of life’ version of BurritoCoin Core is possible, but it might take a while if the data directory needs to be migrated. Old wallet versions of BurritoCoin Core are generally supported.
 

Compatibility:
==============

BurritoCoin Core is supported and extensively tested on operating systems using the Linux kernel, macOS 10.10+,  Windows 7 and newer. It’s not recommended to use BurritoCoin Core on unsupported systems.

BurritoCoin Core should also work on most other Unix-like systems, but is not as frequently tested on them.

From BurritoCoin Core 0.21.1 onwards, macOS versions earlier than 10.12 are no longer supported. Additionally, BurritoCoin Core doesn’t yet change appearance when macOS "dark mode" is activated.
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/BurritoCoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by BurritoCoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. BurritoCoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/BurritoCoin/bips/blob/master/bip-0157.mediawiki) compact
  filters to peers on the network when enabled using
  `-blockfilterindex=1 -peercfilters=1`. (#16442)

- This release implements
  [BIP339](https://github.com/BurritoCoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/BurritoCoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/BurritoCoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `BurritoCoin-cli -netinfo` command provides a network peer connections
...
- A new `BurritoCoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `BurritoCoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `BurritoCoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when BurritoCoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `BurritoCoin-wallet` tool which performs the salvage
...
BurritoCoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`BurritoCoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
BurritoCoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `BurritoCoin-cli createwallet` or `BurritoCoin-wallet
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BRTO per 1,000
  vbytes (BRTO/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BRTO/kB
  to sat/vB and the help documentation in BRTO/kB is updated to BRTO/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BRTO/kvB to sat/vB and the
  ...
  difference between BRTO/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BRTO/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time BurritoCoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `BurritoCoin.conf`
...
- [The BurritoCoin Core Developers]
- Adrian Gallagher
- David Burkett
