BurritoCoin Core version 0.11.0 is now available from:

  <https://burritocoin.org/bin/burritocoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BRTO/kB fee to be
...
For example, add the following to `burritocoin.conf`:
...
related to the blockchain in the burritocoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BurritoCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BurritoCoin Core will 
...
For an example script see [burritocoin-submittx](https://github.com/laanwj/burritocoin-submittx).
...
- #5334 `cf87536` libburritocoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new burritocoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BurritoCoinAmountField
- #5753 `0f44672` Add burritocoin logo to about screen
...
- #5809 `46bfbe7` Add burritocoin-cli man page
...
- #5849 `41259ca` contrib/init/burritocoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BurritoCoin-Qt.app to BurritoCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for burritocoin-tx)
