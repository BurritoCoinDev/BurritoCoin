...
(define* (make-burritocoin-cross-toolchain target
                                  #:key
...
desirable for building BurritoCoin Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-burritocoin-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-burritocoin-cross-toolchain target)))
          (else '())))))
